...
/**
 * Pantalla para solicitar modificación de un perfil genético existente.
 * 
 * Campos:
 * - Nombre del Perfil
 * - Código de Secuencia (ADN)
 * - Descripción
 * - Fecha de Muestra (formato: yyyy-MM-dd)
 * 
 * La estructura es idéntica a Pant_SolicitarPerfil, solo cambia el título
 * y el formato del mensaje enviado al servidor.
 * 
 * @author Admin
 */
public class BancoADN_Grupo6_Pant_SolicitarModPerfil extends javax.swing.JFrame {
    ...
    public BancoADN_Grupo6_Pant_SolicitarModPerfil() {
        initComponents();
        configurarDiseno();
    }
    ...
    /**
     * Deshabilita todos los controles interactivos durante el envío al servidor.
     */
    public void mostrarCargando(boolean cargando) {
        btnEnviarSolicitud.setEnabled(!cargando);
        btnVolver.setEnabled(!cargando);
        txtNombrePerfil.setEnabled(!cargando);
        txtCodigoSecuencia.setEnabled(!cargando);
        txtDescripcion.setEnabled(!cargando);
        txtFechaMuestra.setEnabled(!cargando);
        setCursor(cargando ?
