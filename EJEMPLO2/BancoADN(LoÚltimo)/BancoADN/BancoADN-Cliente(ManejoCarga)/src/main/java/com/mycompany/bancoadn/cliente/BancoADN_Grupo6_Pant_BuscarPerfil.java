...
public class BancoADN_Grupo6_Pant_BuscarPerfil extends JFrame {
    ...
    /**
     * Deshabilita o habilita todos los controles interactivos durante la búsqueda.
     * Cambia el cursor a reloj de espera mientras espera al servidor.
     */
    public void mostrarCargando(boolean cargando) {
        btnBuscar.setEnabled(!cargando);
        btnAtras.setEnabled(!cargando);
        cbFiltro.setEnabled(!cargando);
        txtBusqueda.setEnabled(!cargando);
        setCursor(cargando ?
    ...
    public Map<String, String> getPerfilData(JButton btnVer) { return perfilData.get(btnVer); }
    public List<JButton> getBotonesVer()                     { return new ArrayList<>(botonesVer); }
